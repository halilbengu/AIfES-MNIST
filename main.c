...
void eval_test_data(float*, int);
void calc_success_rate(float*, int);
...
void AIfES_mnist()
{
    ...
    uint16_t test_shape[] = {test_size, 1, 28, 28}; // [sample count, channels, height, width]
	aitensor_t test_tensor = AITENSOR_4D_F32(test_shape, MNIST_test_data);
    uint16_t test_target_shape[] = {test_size, 10};
	...
	ailayer_softmax_f32_t softmax_layer_1   = AILAYER_SOFTMAX_F32_A();
    ...
    x = ailayer_softmax_f32_default(&softmax_layer_1, x);
...
void calc_success_rate(float *output_data, int test_size){
    ...
    float success=0;
    for (int i=0; i < (test_size-1)*10; i+=10){
        predicted_index=0;
        target_index=0;
    ...
    printf("Total Success: %f, Test Size: %d, Success rate: %f\n", success, test_size, success_rate);
}
